...
int threshold = 250;      //Threshold level 
...
void loop() {
  ...
  lcd.print("Air Qualit: ");     
  lcd.print(ppm);                

  if (ppm > threshold)            // check threshold level
    {
      lcd.setCursor(1,1);         
      lcd.print("AQ Level HIGH");
      Serial.println("AQ Level HIGH");     
      tone(led,1000,200);         //LED-> turn on time 1000mS, turn off time 200mS
